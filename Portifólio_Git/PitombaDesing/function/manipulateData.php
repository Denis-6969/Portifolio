...
function update($tabela, $valor, $campoId, $valorId){
    try {
        ...
        return query($sql) ? "Atualizado com sucesso" : "Erro ao atualizar!";
    } catch (Exception $e) {
        return "Erro de SQL: {$e->getMessage()} <p>SQL: </p> ";
    }
}
/**Função para atualizar somente a assinatura */
function update2($tabela, $valor, $campoId, $valorId){
    try {
        $sql = "UPDATE $tabela SET $valor WHERE $campoId = $valorId";
        return query($sql) ? "" : "Erro ao atualizar os dados!";
    } catch (Exception $e) {
        return "Erro de SQL: {$e->getMessage()} <p>SQL: </p> ";
    }
}
...
function ifExist($tabela, $campoId, $valorId){
    try {
